...
**NB based on a fork from: https://github.com/bcardiff/docker-rclone (credits!)**
...
```
$ mkdir config
$ docker run --rm -it -v $(pwd)/config:/config justb4/rclone-sync:<version>
```
...
```bash
$ docker run --rm -it -v $(pwd)/config:/config -v /path/to/source:/source -e SYNC_SRC="/source" -e SYNC_DEST="dest:path" -e TZ="America/Argentina/Buenos_Aires" -e CRON="0 0 * * *" -e CRON_ABORT="0 6 * * *" -e FORCE_SYNC=1 -e CHECK_URL=https://hchk.io/hchk_uuid justb4/rclone-sync:<version>
```
